//
//  PWKVStorage.h
//  PWCache <https://github.com/ibireme/PWCache>
//
...
/**
 PWKVStorageItem is used by `PWKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface PWKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `PWKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use PWKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use PWKVStorageTypeFile to get better performance.
 * You can use PWKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, PWKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    PWKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    PWKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    PWKVStorageTypeMixed = 2,
};



/**
 PWKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for PWKVStorage is `initWithPath:type:`. 
 ...
 */
@interface PWKVStorage : NSObject
...
@property (nonatomic, readonly) PWKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(PWKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is PWKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is PWKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is PWKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(PWKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 PWKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is PWKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is PWKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is PWKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable PWKVStorageItem *)getItemForKey:(NSString *)key;

/**
 Get path with a specified key.
 
 @param key A specified key.
 @return Path for the key, or nil if not exists / error occurs.
 */
 ...
 */
- (nullable PWKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `PWKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<PWKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `PWKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<PWKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
