...
- (NSURL *)encodedVideoFileURLForKey:(NSString *)key
{
    if (![key length])
        return nil;
    ...
    //Significantly improve performance by indicating that the URL will *not* result in a directory.
    //Also note that accessing _cacheURL is safe without the lock because it is only set on init.
    return [_cacheURL URLByAppendingPathComponent:[[self encodedString:key] stringByAppendingString:@".mp4"] isDirectory:NO];
}
...
- (NSURL *)videoFileURLForKey:(NSString *)key
{
    if (!key) {
        return nil;
    }
    ...
    NSDate *now = [[NSDate alloc] init];
    NSURL *fileURL = [self encodedVideoFileURLForKey:key];
...
- (void)setVideoData:(NSData* )data forKey:(NSString *)key
{
    ...
    if (willAddObjectBlock) {
        [self unlock];
        willAddObjectBlock(self, key, data);
        [self lock];
    }
    ...
    if (didAddObjectBlock) {
        [self unlock];
        didAddObjectBlock(self, key, data);
        [self lock];
    }
    [self unlock];
}
