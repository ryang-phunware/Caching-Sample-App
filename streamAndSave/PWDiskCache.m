...
static int64_t _PWDiskSpaceFree() {
    NSError *error = nil;
...
/// String's md5 hash.
static NSString *_PWNSStringMD5(NSString *string) {
...
static void _PWDiskCacheInitGlobal() {
...
static PWDiskCache *_PWDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _PWDiskCacheInitGlobal();
...
static void _PWDiskCacheSetGlobal(PWDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _PWDiskSpaceFree();
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (!filename) filename = _PWNSStringMD5(key);
    return filename;
}
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"PWDiskCache init error" reason:@"PWDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    PWDiskCache *globalCache = _PWDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    PWKVStorageType type;
    if (threshold == 0) {
        type = PWKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = PWKVStorageTypeSQLite;
    } else {
        type = PWKVStorageTypeMixed;
    }
    
    PWKVStorage *kv = [[PWKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    PWKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [PWDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [PWDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != PWKVStorageTypeSQLite) {
...
- (void)setData:(NSData *)data forKey:(NSString *)key {
    ...
    Lock();
    [_kv saveItemWithKey:key value:data filename:filename extendedData:nil];
    Unlock();
}
