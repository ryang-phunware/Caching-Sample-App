...
static UIApplication *_PWSharedApplication() {
...
@implementation PWKVStorageItem
@end

@implementation PWKVStorage {
...
- (PWKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    PWKVStorageItem *item = [PWKVStorageItem new];
...
- (PWKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    PWKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            PWKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"PWKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:PWKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(PWKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"PWKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > PWKVStorageTypeMixed) {
        NSLog(@"PWKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"PWKVStorage init error:%@", error);
        return nil;
    }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"PWKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_PWSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_PWSharedApplication() endBackgroundTask:taskID];
    }
}
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == PWKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != PWKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case PWKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case PWKVStorageTypeFile:
        case PWKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (PWKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (PWKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    PWKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (NSString *)getPathForKey:(NSString *)key {
    ...
    if (item) {
        [self _dbUpdateAccessTimeWithKey:key];
        if (item.filename) {
            path = [_dataPath stringByAppendingPathComponent:item.filename];
        }
    }
    return path;
}

- (PWKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    PWKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
