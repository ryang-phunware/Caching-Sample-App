...
/**
 PWDiskCache is a thread-safe cache that stores key-value pairs backed by SQLite
 and file system (similar to NSURLCache's disk cache).
 
 PWDiskCache has these features:
 ...
 */
@interface PWDiskCache : NSObject
...
/**
 Returns the path associated with a given key.
 This method may blocks the calling thread until file read finished.
 
 @param key A string identifying the value. If nil, just return nil.
 @return The path associated with key, or nil if no value is associated with key.
 */
- (nullable NSString *)pathForKey:(NSString *)key;
