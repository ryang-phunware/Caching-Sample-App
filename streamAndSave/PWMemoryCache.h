...
/**
 PWMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 PWMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in PWMemoryCache is typically in constant time (O(1)).
 */
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(PWMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(PWMemoryCache *cache);
